...
void mpegDetectAndDumpPrivateStream(FILE *f, unsigned int offset, FILE *aout, FILE *sout, FILE *pout, char *name)
{
	...
	case ZOEPT_VAG:
	{ // stream 0x01
		printf("STREAMID ZOEPT_VAG 0x01\n");
		if (ftell(aout) == 0)
		{
			...
			audioSamplerate[2] = sonyPrivateHead.samplerate[0];
			audioSamplerate[3] = sonyPrivateHead.samplerate[1];
			audioNumChannels = sonyPrivateHead.channels;
			unsigned char null[0x40] = {0};
			if (audioNumChannels == 1)
				fwrite(null, 1, 0x30, aout);
			else if (audioNumChannels == 2)
				fwrite(null, 1, 0x40, aout);
		}
		else
		{
	...
	case ZOEPT_DMX:
	{ // stream 0x05
		printf("STREAMID ZOEPT_DMX 0x05\n");
		fseek(f, -0x10, SEEK_CUR);
		writeFile(f, headSize, pout);
		break;
	}
	default:
	{ // subtitle streams, always seem to fit into one PES
		printf("STREAMID IDK %08x\n", sonyExtHead.streamId);
...
void vagPrepareHeader(vagHeader *header, uint32_t size, char *name)
{
	...
	header->samplerate[1] = 0;
	header->samplerate[2] = audioSamplerate[2];
	header->samplerate[3] = audioSamplerate[3];
	memset(header->reserved1, 0, 4);
	memset(header->reserved2, 0, 12);
	if (audioNumChannels == 1)
		strncpy(header->magic, "VAGp", 4);
	else if (audioNumChannels == 2)
		strncpy(header->magic, "VAG2", 4);
	return;
}
...
int main(int argc, char **argv)
{
	...
	sprintf(outfilename, "%s-stream00test.bin", argv[1]);
	...
	//		printf("Couldnt delete unused audio stream %s\n", outfilename);
	//}
	//if (!haveVideo)
	//{
	//	sprintf(outfilename, "%s-stream00.m2v", argv[1]);
	//	if (remove(outfilename))
	//		printf("Couldnt delete unused video file %s\n", outfilename);
	//}
	//if (!havePoly)
	//{
	//	sprintf(outfilename, "%s-stream05.dmx", argv[1]);
	//	if (remove(outfilename))
	//		printf("Couldnt delete unused poly file %s\n", outfilename);
	//}
